...
b642hex = lambda s: binascii.hexlify(base64.b64decode(s))

ascii2b = lambda s: bytes([ord(i) for i in s])
...
for i in range(2, 40):
    ...
    for j in range(len(data) // 40):
        dist.append(
            hamDist(data[j * i : (j + 1) * i], data[(j + 1) * i : (j + 2) * i])
            / float(i)
        )
...
for n in range(3):
    ...
    if printable(key[0].encode()):
        print(decodeRepeatingXor(b2hex(data), key).decode())
